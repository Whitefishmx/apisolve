	...
	class ConciliacionModel extends Model {
		...
		public function makeConciliationPlus ( array $args, string $env = NULL ) {
			...
			foreach ( $args as $row ) {
				...
				$data = [ $nexID,  $user[ 'id' ], $companies[ 'client' ][ 'id' ], $companies[ 'provider' ][ 'id' ], strtotime ( 'now' )];
				$folio = serialize32 ( $data );
				$paymentDate = strtotime ('+40 days');
				$query = "INSERT INTO apisolve_sandbox.conciliation_plus (invoice_range, id_client, id_provider, reference_number, folio, entry_money, exit_money, payment_date, status)
